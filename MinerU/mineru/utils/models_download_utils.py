...
# 探测 URL 走 HF_ENDPOINT（如 https://hf-mirror.com），默认回退到 huggingface.co；
# 与 huggingface_hub.snapshot_download 保持一致的镜像行为，方便国内/受限网络。
HF_ENDPOINT_ENV_VAR = 'HF_ENDPOINT'
...
def resolve_auto_model_source() -> str:
    ...
    last_error = None
    probe_url = (
        os.getenv(HF_ENDPOINT_ENV_VAR, 'https://huggingface.co').rstrip('/') + '/models'
    )
    ...
    logger.warning(
        f"Failed to access {probe_url}: {last_error}, fallback to modelscope."
    )
    return "modelscope"
